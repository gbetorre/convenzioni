...
public class DBWrapper extends QueryImpl {
    ...
    /**
     * <p>Restituisce la lista delle convenzioni attive 
     * in base a una o pi&uacute; chiavi di ricerca passate dall'utente.</p>
     *
     * @param user utente che ha effettuato la richiesta
    ...
    public ArrayList<Convenzione> getConventions(PersonBean user,
        ...
        try (Connection con = col_manager.getConnection()) {
            ...
            try {
                ...
                pst = con.prepareStatement(query);
                pst.clearParameters();
                // TODO Per il momento, ignora i gruppi dell'utente: le convenzioni escono nella ricerca ma i link verranno bloccati
                rs = pst.executeQuery();
                while (rs.next()) {
                    c = new Convenzione();
                    BeanUtil.populate(c, rs);
    ...
    public ArrayList<Convenzione> getConventions(PersonBean user,
        ...
        try (Connection con = col_manager.getConnection()) {
            ...
            ArrayList<Convenzione> convenzioni = new ArrayList<>();
            try {
                pst = con.prepareStatement(GET_CONVENTIONS_BY_CONTRACTOR);
                pst.clearParameters();
                pst.setInt(++nParam,contractor.getId());
                // TODO Per il momento, ignora i gruppi dell'utente: le convenzioni escono nella ricerca ma i link verranno bloccati
                rs = pst.executeQuery();
                while (rs.next()) {
                    c = new Convenzione();
    ...
    /**
     * <p>Restituisce i dettagli di un contraente.</p>
     *
     * @param user utente che ha effettuato la richiesta
     * @param id identificativo del contraente
     * @return <code>ArrayList&lt;PersonBean&gt;</code> - lista contraenti trovati
     * @throws it.col.exception.WebStorageException se si verifica un problema nell'esecuzione della query, nell'accesso al db o in qualche tipo di puntamento
     * @throws it.col.exception.AttributoNonValorizzatoException  eccezione che viene sollevata se questo oggetto viene usato e l'id della persona non &egrave; stato valorizzato (&egrave; un dato obbligatorio)
     */
    public PersonBean getContractor(PersonBean user,
                                    int id)
                             throws WebStorageException, 
                                    AttributoNonValorizzatoException {
        try (Connection con = col_manager.getConnection()) {
            ...
            try {
                // TODO: Controllare i diritti dell'utente
                pst = con.prepareStatement(GET_CONTRACTOR);
                pst.clearParameters();
                pst.setInt(++numParam, id);
                rs = pst.executeQuery();
                if (rs.next()) {
                    c = new PersonBean();
                    BeanUtil.populate(c, rs);
                    convenzioni = this.getConventions(user, c);
                    c.setConvenzioni(convenzioni);
                }
                // Try to engage the Garbage Collector
                pst = null;
                // Get Out
    ...
    /**
     * <p>Metodo per fare l'inserimento di una nuova convenzione 
     * definita dall'utente.</p>
     *  
     * @param user      utente loggato
     * @param params    mappa contenente i parametri di navigazione
     ...
     * @throws WebStorageException se si verifica un problema nel cast da String a Date, nell'esecuzione della query, nell'accesso al db o in qualche puntamento
     */
    public Convenzione insertConvention(PersonBean user, 
                                        HashMap<String, LinkedHashMap<String, String>> params) 
                                 throws WebStorageException {
        try (Connection con = col_manager.getConnection()) {
            ...
            int index = NOTHING;
            // Definisce l'ID della convenzione da inserire
            int idConv = getMax("convenzione") + ELEMENT_LEV_1;
            // Return type
            ...
            try {
                ...
                con.setAutoCommit(false);
                // TODO: Controllare se user è superuser
                pst = con.prepareStatement(INSERT_CONVENTION);
                pst.clearParameters();
                // === ID ===
                pst.setInt(++nextParam, idConv);
                c.setId(idConv);
                ...
                pst.setInt(++nextParam, user.getUsrId());
                // === Id tipo ===
                String tipoAsString = new String(convention.get("type"));
                // Explicit unboxing of a String into int
                int tipo = Integer.valueOf(tipoAsString).intValue();
                pst.setInt(++nextParam, tipo);
                // Gestire la conversione di Id in etichetta, se serve
                c.setTipo(tipoAsString);
                // === Id stato ===
                pst.setInt(++nextParam, ELEMENT_LEV_1); // <- stato ATTIVO
                // === Id convenzione ===
                // TODO: gestire self-relationship in caso rinnovo (id_convenzione = id conv. originale)
                // Do Insert agreement: ==>
                pst.executeUpdate();
                // Preparazione inserimento relazioni
                try {
                    // Query di inserimento gruppi associati
                    pst1 = con.prepareStatement(INSERT_CONVENTION_GROUP);
                    pst1.clearParameters();
                    Vector<CodeBean> usrGrps = user.getGruppi();
                    for (CodeBean grp : usrGrps) {
                        // Numero di parametro da passare alla query
                        nextParam = NOTHING;
                        // === Id Convenzione ===
                        pst1.setInt(++nextParam, idConv);
                        // === id_grp ===
                        pst1.setInt(++nextParam, grp.getId());
                        // notifica
                        pst1.setBoolean(++nextParam, GET_ALL);
                        // selezione
                        pst1.setBoolean(++nextParam, GET_ALL);
                        // aggiornamento
                        pst1.setBoolean(++nextParam, GET_ALL);
                    ...
                    // Do Insert grps
                    int[] updateGrps = pst1.executeBatch();
                    LOG.info(updateGrps.length + " relazioni su gruppi in transazione attiva.\n");
                    // Do Insert scopes
                    int[] updateScopes = pst2.executeBatch();
                    LOG.info(updateScopes.length + " relazioni su finalita\' in transazione attiva.\n");
                } catch (NumberFormatException nfe) {
                    String msg = FOR_NAME + "Si e\' verificato un problema nella conversione di interi.\n" + nfe.getMessage();
                    LOG.severe(msg);
                    throw new WebStorageException(msg, nfe);
        ...
        try (Connection con = col_manager.getConnection()) {
            ...
            try {
                ...
                if (rs.next()) {    // Esiste già un accesso: lo aggiorna
                    ...
                    pst.clearParameters();
                    int nextVal = getMax("access_log") + ELEMENT_LEV_1;
